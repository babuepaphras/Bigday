 ...
 *  
 */
package de.hybris.bigday.cockpits.setup;
...
import de.hybris.bigday.cockpits.constants.BigdayCockpitsConstants;
 ...
 */
@SystemSetup(extension = BigdayCockpitsConstants.EXTENSIONNAME)
public class CockpitSystemSetup extends AbstractSystemSetup
{
	...
	public void createEssentialData(final SystemSetupContext context)
	{
		...
		{
			if (MediaManager.getInstance().getMediaFolderByQualifier(BigdayCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER).size() < 1)
			{
				MediaManager.getInstance().createMediaFolder(BigdayCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER,
						BigdayCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER);
			}
			try
			{
				String prefix = null;
				...
				if (prefix != null){
					importImpexFile(context, "/bigdaycockpits/reportcockpit/import/" + prefix + "_jasperreports.impex");
				}
			}
